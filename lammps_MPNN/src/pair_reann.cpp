...
void PairREANN::init_style()
{
    ...
    if (datatype=="double") tensor_type=torch::kFloat64;
    else 
    {
...
void PairREANN::compute(int eflag, int vflag)
{
    ...
    int i,ii,inum,j,jj,jnum,maxneigh,center_num;
    ...
    auto centerlist_=torch::from_blob(centerlist.data(),{totneigh},option2).to(device_tensor.device(),true);
    auto neighlist_=torch::from_blob(neighlist.data(),{totneigh},option2).to(device_tensor.device(),true);
    auto neigh_species_=torch::from_blob(neigh_species.data(),{totneigh},option2).to(device_tensor.device(),true);
    auto local_species_=torch::from_blob(local_species.data(),{totneigh},option2).to(device_tensor.device(),true);
    auto center_neighlist_=torch::from_blob(center_neighlist.data(),{totneigh},option2).to(device_tensor.device(),true);
    auto outputs = module.forward({cart_,centerlist_,neighlist_,local_species_,neigh_species_,center_neighlist_,numlocal}).toTuple()->elements();
